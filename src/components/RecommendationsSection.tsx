...
const RecommendationsSection = () => {
  return (
    <section className="py-12 px-6 relative overflow-hidden" id="recommendations">
      <div className="absolute inset-0">
        <img src={testimonialBg} alt="" className="w-full h-full object-cover opacity-[0.05]" />
        <div className="absolute inset-0 bg-gradient-to-b from-background via-background/95 to-background" />
      </div>

      <div className="absolute top-20 left-10 w-80 h-80 rounded-full bg-[#0077B5]/[0.06] blur-3xl" />
      <div className="absolute bottom-10 right-10 w-96 h-96 rounded-full bg-[#0077B5]/[0.04] blur-3xl" />
      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {recommendations.map((rec) => (
            <div key={rec.name} className="group relative">
